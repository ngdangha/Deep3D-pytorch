...
def replaceImageNameToText(file):
	return file.split(os.path.sep)[-1].replace('.png','.txt').replace('.PNG','.txt').replace('.jpg','.txt').replace('.JPG','.txt')
...
for individual_path in train_val_path:
	...
	for file in img_list:
		...
		available_detection_txt = Path(image_path, landmark_path, landmark_save_name)

		if available_detection_txt.is_file() and (not os.stat(available_detection_txt).st_size == 0):
			print(bcolors.WARNING + str(n) + " --- Landmark " + str(landmark_save_name) + " existed. Skipping...")
			continue
		...
		keypoints = detect_result[0]['keypoints']
		...
		print(bcolors.OKGREEN + str(n) + " --- Ok ")
